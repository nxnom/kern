...
export function drawSheet(
  ...
  // showed a pair in company was the one it rarely asked for.
  context = true,
): Sheet {
  const { glyph: GLYPH_PX, cellH: CELL_H } = SHEET_SIZES[size]
  const scale = GLYPH_PX / lf.unitsPerEm

  /** The glyphs a cell actually draws, pair plus company. */
  const layout = (item: SheetItem) => {
    const l = lf.font.charToGlyph(item.left)
    const r = lf.font.charToGlyph(item.right)
    const pre = context ? lf.font.charToGlyph(flankFor(item.left)) : null
    const post = context ? lf.font.charToGlyph(flankFor(item.right)) : null
    const preAdv = pre?.advanceWidth ?? 0
    const lAdv = l?.advanceWidth ?? 0
    const rAdv = r?.advanceWidth ?? 0
    const postAdv = post?.advanceWidth ?? 0
    return {
      l, r, pre, post, preAdv, lAdv, rAdv, postAdv,
      totalPx: (preAdv + lAdv + item.kern + rAdv + postAdv) * scale,
    }
  }

  // Sized to the widest cell this sheet actually holds, not to a guess. A flat
  // allowance based on `n` looked right and clipped `HWWH` by a few pixels,
  // which is exactly the kind of thing that goes unnoticed in a thumbnail.
  const GUTTER = Math.round(GLYPH_PX * 0.3)
  const widest = items.reduce((w, it) => Math.max(w, layout(it).totalPx), 0)
  const CELL_W = Math.max(SHEET_SIZES[size].cellW, Math.ceil(widest + GUTTER))
  ...
  items.forEach((item, i) => {
    ...
    const {
      l: lGlyph, r: rGlyph, pre: preGlyph, post: postGlyph,
      preAdv, lAdv, rAdv, totalPx,
    } = layout(item)
